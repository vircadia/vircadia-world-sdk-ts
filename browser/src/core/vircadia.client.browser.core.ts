...
export class WsConnectionCore {
    ...
    private channelListeners = new Map<
        string,
        {
            ...
            listener: (
                msg:
                    | Communication.WebSocket.EntityDeliveryMessage
                    | Communication.WebSocket.EntityMetadataDeliveryMessage,
            ) => void;
        }
    >();
    private channelServerSubscriptions = new Map<string, number>();
    ...
    disconnect(): void {
        ...
        this.channelServerSubscriptions.clear();
    ...
    private async sendEntityChannelSubscribe(
        entityName: string,
        channel: string | null,
        timeoutMs?: number,
    ): Promise<void> {
        ...
        const parsed =
            Communication.WebSocket.Z.EntityChannelSubscribeRequest.safeParse(
                messageData,
            );
        if (!parsed.success) {
            throw new Error(
                `Invalid entity channel subscribe request: ${parsed.error.message}`,
            );
        }
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            this.pendingRequests.set(requestId, {
                resolve: (value: unknown) => {
                    ...
                    this.pendingRequests.delete(requestId);
                    const response =
                        value as Communication.WebSocket.EntityChannelSubscribeResponseMessage;
    ...
    private async sendEntityChannelUnsubscribe(
        entityName: string,
        channel: string | null,
        timeoutMs?: number,
    ): Promise<void> {
        ...
        const parsed =
            Communication.WebSocket.Z.EntityChannelUnsubscribeRequest.safeParse(
                messageData,
            );
        if (!parsed.success) {
            throw new Error(
                `Invalid entity channel unsubscribe request: ${parsed.error.message}`,
            );
        }
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            this.pendingRequests.set(requestId, {
                resolve: (value: unknown) => {
                    ...
                    this.pendingRequests.delete(requestId);
                    const response =
                        value as Communication.WebSocket.EntityChannelUnsubscribeResponseMessage;
    ...
    async subscribeEntityChannel(data: {
        entityName: string;
        channel?: string | null;
        listener: (
            msg:
        ...
        if (typeof data.listener !== "function") {
            throw new Error(
                "Listener is required for entity channel subscription",
            );
        }
        ...
        const channel = data.channel === undefined ? null : data.channel;
        const subscriptionKey = `${entityName}|${channel ?? "*"}`;
        const current =
            this.channelServerSubscriptions.get(subscriptionKey) ?? 0;
        if (current === 0) {
            await this.sendEntityChannelSubscribe(
                entityName,
                channel,
                data.timeoutMs,
            );
        }
        this.channelServerSubscriptions.set(subscriptionKey, current + 1);

        const handleId = crypto.randomUUID();
        this.channelListeners.set(handleId, {
            entityName,
            channel,
        ...
        return {
            remove: async () => {
                if (!this.channelListeners.delete(handleId)) {
                    return;
                }
                const remaining = this.channelServerSubscriptions.get(
                    subscriptionKey,
                );
                if (!remaining) {
                    return;
                ...
                if (remaining <= 1) {
                    try {
                        await this.sendEntityChannelUnsubscribe(
                            entityName,
                            channel,
                            data.timeoutMs,
                        );
                    } finally {
                        this.channelServerSubscriptions.delete(subscriptionKey);
                    }
                } else {
                    this.channelServerSubscriptions.set(
                        subscriptionKey,
                        remaining - 1,
                    );
                }
    ...
    private handleMessage(event: MessageEvent): void {
        try {
            ...
            if (
                ...
                const listeners = Array.from(this.channelListeners.values());
                const messageChannel = message.channel ?? null;
                for (const entry of listeners) {
                    if (entry.entityName !== message.entityName) {
                        continue;
                    }
                    // Deliver if:
                    // - Subscribed to "all channels" (entry.channel === null) OR
                    // - Subscribed to the specific channel that matches the message channel OR
                    // - Message has no channel (null) and entry is subscribed to "all channels"
                    const shouldDeliver =
                        entry.channel === null ||
                        (messageChannel !== null &&
                            entry.channel === messageChannel) ||
                        (messageChannel === null && entry.channel === null);
                    if (shouldDeliver) {
...
export class RestAuthCore {
    ...
    async authorizeOAuth(provider: string): Promise<OAuthAuthorizeResponse> {
        ...
        // Include the current website origin as redirectUri so the server can validate and use it
