...
interface ClientCoreConfig {
    // Connection settings
    /** URL of the Vircadia API WebSocket server to connect to */
    apiWsUri: string;
    /** URL of the Vircadia API REST server to connect to */
    apiRestAuthUri: string;
    /** URL of the Vircadia API Asset server to connect to */
...
class CoreConnectionManager {
    ...
    private async validateSessionViaRest(
        restManager: CoreRestAuthManager,
    ): Promise<void> {
        ...
        try {
            ...
            let status: WsConnectionCoreAuthStatus;
            if (isValid) {
                status = "valid";
     ...
     * @param {WsConnectionCoreState} status - The new connection status
     * @private
     */
     ...
     * @returns {Promise<WsConnectionCoreInfo>} - Promise resolving to connection information
        ...
        this.connectionPromise = (async () => {
            try {
                ...
                console.log(
                    ...
                    {
                        originalUrl: this.config.apiWsUri,
     ...
     * @returns {WsConnectionCoreInfo} Current connection information
     */
...
/**
 * Handles Vircadia REST authentication endpoints
 */
class CoreRestAuthManager {
    constructor(private config: ClientCoreConfig) {}
...
/**
 * Handles Vircadia REST asset endpoints
 */
class CoreRestAssetManager {
    constructor(private config: ClientCoreConfig) {}
...
export class ClientCore {
    ...
    private coreRestAuthManager: CoreRestAuthManager;
    private coreRestAssetManager: CoreRestAssetManager;
    ...
    constructor(private config: ClientCoreConfig) {
        ...
        this.coreRestAuthManager = new CoreRestAuthManager(config);
        this.coreRestAssetManager = new CoreRestAssetManager(config);
    }

    /**
     * Updates the API WebSocket server URL.
     * @param {string} url - The new API WebSocket server URL.
     */
    setApiWsUri(url: string): void {
        this.config.apiWsUri = url;
        this.coreConnectionManager.updateConfig(this.config);
        this.coreRestAuthManager.updateConfig(this.config);
        this.coreRestAssetManager.updateConfig(this.config);
    }

    /**
     * Updates the API REST server URL.
     * @param {string} url - The new API REST server URL.
     */
    setApiRestAuthUri(url: string): void {
        this.config.apiRestAuthUri = url;
        this.coreRestAuthManager.updateConfig(this.config);
    }

    /**
     * Updates the API Asset server URL.
     * @param {string} url - The new API Asset server URL.
     */
    setApiRestAssetUri(url: string): void {
        this.config.apiRestAssetUri = url;
        this.coreRestAssetManager.updateConfig(this.config);
    }
    ...
    get Utilities() {
        ...
        const rmAuth = this.coreRestAuthManager;
        const rmAsset = this.coreRestAssetManager;

        return {
            Connection: {
                connect: (options?: { timeoutMs?: number }) =>
                    cm.connect({ ...options, restManager: rmAuth }),
            ...
            RESTAuth: {
                validateSession: rmAuth.validateSession.bind(rmAuth),
                loginAnonymous: rmAuth.loginAnonymous.bind(rmAuth),
                authorizeOAuth: rmAuth.authorizeOAuth.bind(rmAuth),
                handleOAuthCallback: rmAuth.handleOAuthCallback.bind(rmAuth),
                logout: rmAuth.logout.bind(rmAuth),
                linkProvider: rmAuth.linkProvider.bind(rmAuth),
                unlinkProvider: rmAuth.unlinkProvider.bind(rmAuth),
                listProviders: rmAuth.listProviders.bind(rmAuth),
            },
            RESTAsset: {
                assetGetByKey: rmAsset.assetGetByKey.bind(rmAsset),
            },
        };
    }
